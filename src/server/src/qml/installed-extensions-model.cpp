...
QVariantList InstalledExtensionsSection::displayAccessories(const ExtensionManifest &m) const {
  ...
  if (m.isFromSeronStore()) {
    return qml::accessoriesToVariantList({{.data = AccessoryModel::Tag(SemanticColor::Orange, "Seron"),
                                           .icon = ExtensionImageModel{.source = QString("seron")}}});
  }
  if (m.isLocal()) {
