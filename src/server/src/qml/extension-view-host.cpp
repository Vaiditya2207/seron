...
QUrl ExtensionViewHost::qmlComponentUrl() const {
  return QUrl(QStringLiteral("qrc:/Seron/ExtensionView.qml"));
}

QUrl ExtensionViewHost::qmlSearchAccessoryUrl() const {
  if (!m_dropdownItems.isEmpty()) return QUrl(QStringLiteral("qrc:/Seron/ExtensionDropdownAccessory.qml"));
  if (!m_linkAccessoryText.isEmpty()) return QUrl(QStringLiteral("qrc:/Seron/FormLinkAccessory.qml"));
  return {};
}
