...
LauncherWindow::LauncherWindow(ApplicationContext &ctx, QObject *parent)
  ...
  // Ensure Wayland app_id / X11 WM_CLASS is "seron"
  QGuiApplication::setDesktopFileName(QStringLiteral("seron"));
  ...
  m_engine.load(QUrl(QStringLiteral("qrc:/Seron/LauncherWindow.qml")));
  ...
  if (!Environment::isHudDisabled()) {
    ...
    m_engine.load(QUrl(QStringLiteral("qrc:/Seron/HudWindow.qml")));
  }
