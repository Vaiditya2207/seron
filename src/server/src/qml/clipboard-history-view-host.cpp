...
QUrl ClipboardHistoryViewHost::qmlComponentUrl() const {
  return QUrl(QStringLiteral("qrc:/Seron/ClipboardHistoryView.qml"));
}

QUrl ClipboardHistoryViewHost::qmlSearchAccessoryUrl() const {
  return QUrl(QStringLiteral("qrc:/Seron/ClipboardFilterAccessory.qml"));
}
  ...
  if (!data) {
    ...
    case ClipboardService::OfferDecryptionError::DecryptionFailed:
      ...
      m_detailErrorDescription = QStringLiteral(
          "Seron could not decrypt the data for this selection. This is most likely caused by a "
