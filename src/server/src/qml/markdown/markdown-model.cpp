...
QString imageProviderUrl(const QString &rawUrl) {
  if (rawUrl.startsWith(QStringLiteral("image://seron/"))) return rawUrl;

  QUrl const url(rawUrl);
  auto scheme = url.scheme();
  if (scheme == "https" || scheme == "http") return QStringLiteral("image://seron/http:") + rawUrl;
  if (scheme == "data") return QStringLiteral("image://seron/datauri:") + rawUrl;
  if (scheme == "file") {
    auto path = url.host().isEmpty() ? url.path() : url.host() + url.path();
    return QStringLiteral("image://seron/local:") + path;
  }
