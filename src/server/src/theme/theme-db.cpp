...
void ThemeDatabase::scan() {
  ...
  auto defaultDark = std::make_shared<ThemeFile>(ThemeFile::seronDark());
  auto defaultLight = std::make_shared<ThemeFile>(ThemeFile::seronLight());
  ThemeParser parser;
...
std::vector<std::filesystem::path> ThemeDatabase::defaultSearchPaths() {
  ...
  auto suffix = fs::path("seron") / "themes";
