...
void SnippetServer::start() {
  const auto path = seron::findHelperProgram("seron-snippet-server");

  if (!path) {
    qWarning() << "could not find seron-snippet-server helper binary, snippet expansion will not work";
    return;
  }
...
void SnippetServer::injectClipboardText(std::string_view trigger, QString text, bool terminal) {
  ...
  expansionData->setData("seron/concealed", "1");
