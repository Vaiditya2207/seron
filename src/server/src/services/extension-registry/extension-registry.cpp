...
ExtensionRegistry::ExtensionRegistry(LocalStorageService &storage) : m_storage(storage) {
  ...
  // An extension should be removed from within Seron directly so that other cleanup tasks
  // can be performed.
...
fs::path ExtensionRegistry::localExtensionDirectory() { return xdgpp::dataHome() / "seron" / "extensions"; }

fs::path ExtensionRegistry::supportDirectory() { return xdgpp::dataHome() / "seron" / "support"; }
...
std::vector<fs::path> ExtensionRegistry::extensionDirectories() {
  ...
  for (const auto &dir : dd) {
    fs::path const extDir = dir / "seron" / "extensions";
