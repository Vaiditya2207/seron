...
public:
  QUrl qmlComponentUrl() const override { return QUrl(QStringLiteral("qrc:/Seron/StoreIntroView.qml")); }
...
This is a regular paragraph with **bold text**, *italic text*, and ***bold italic***. Here is some `inline code` and a [link to Seron](https://seron.com). You can also use ~~strikethrough~~ text.
...
![Seron icon](image://seron/builtin:cart?raycast-width=32&raycast-height=32)
...
![HTTP GIF](https://docs.seron.com/scripts/full-output-demo.gif)
