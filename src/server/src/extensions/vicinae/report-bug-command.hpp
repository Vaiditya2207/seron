...
class ReportSeronBugCommand : public BuiltinUrlCommand {

  QString id() const override { return "report-bug"; }
  QString name() const override { return "Report a Seron Bug"; }

  QString description() const override {
    return "Navigate to Seron issue creation page with all relevant informations pre-filled.";
  }
  ...
  QUrl url(const ArgumentValues &values) const override {
    ...
    QString content = ISSUE_TEMPLATE.arg(SERON_GIT_TAG)
                          .arg(SERON_GIT_COMMIT_HASH)
                          .arg(BUILD_INFO)
                          .arg(SERON_PROVENANCE)
                          .arg(osString)
