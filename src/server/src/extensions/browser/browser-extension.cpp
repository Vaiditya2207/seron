...
static bool guard(CommandController *ctrl) {
  ...
  if (browserService->browsers().empty()) {
    auto empty = new EmptyViewHost(
        ...
        "You need to connect at least one browser to seron using the browser extension in "
    ...
    main->addAction(
        new OpenInBrowserAction(QUrl("https://docs.seron.com/browser-extension"), "Open documentation"));
...
class CreateShortcutFromActiveBrowserTabCommand : public GuardedBuiltinCallbackCommand {
  ...
  QString description() const override {
    return "Create a seron shortcut from the currently active browser tab. May yield unexpected results if "
