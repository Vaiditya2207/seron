...
Seron provides direct integration with the official [Raycast store](https://www.raycast.com/store), allowing you to search and install Raycast extensions directly from Seron.

Each extension has a colored compatibility indicator showing how well it works on Linux.

Seron also has its own [extension store](seron://extensions/seron/core/store), which does not suffer from these limitations.
)");
      auto icon = iconUrl();
