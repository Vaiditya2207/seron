...
public:
   ...
   * We first look for the first `seron-node` binary in PATH and then do the same for `node`.
   * `seron-node` is used by script installations and have a specific name to ensure we do not
