...
static constexpr const char *TOP_COMMENT =
    R"(// This configuration is merged with the default seron configuration file, which you can obtain by running the `seron config default` command.
...
//
// You can make manual edits to this file, however you should keep in mind that this file may be written to by seron when a configuration change is made through the GUI.
...
//
// Learn more about configuration at https://docs.seron.com/config)";
...
Manager::Manager(fs::path path) : m_userPath(std::move(path)) {
  ...
  if (const char *envOverrides = std::getenv("SERON_OVERRIDES")) {
    m_envOverrides =
    ...
    qInfo() << "Loaded" << m_envOverrides.size() << "path(s) from SERON_OVERRIDES";
