...
`libemoji` is a very simple library providing a simple API to work with emojis. It is used by seron to provide emoji picking capabilities and decide when to render emoji icons.
...
The seron extension ecosystem allows extensions to make use of regular emoji strings as valid icons. For that to work reliably, we need a way to identify whether a given string is an emoji or something else entirely.
