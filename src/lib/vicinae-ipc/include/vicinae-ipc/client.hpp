...
public:
  static std::filesystem::path seronSocketPath() {
...
private:
  Client(int fd, const std::string &path = seronSocketPath()) : m_sock(fd), m_path(path) {}
...
/**
 * Client to communicate from a browser extension native host to the seron daemon
 */
