...
std::vector<fs::path> helperProgramCandidates(std::string_view program) {
  ...
  return {
      self / program,
      self.parent_path() / SERON_LIBEXECDIR / program,
      fs::path{SERON_LIBEXEC_PATH} / program,
  };
}
...
/**
 * Seron relies on a lot of small programs in order to provide various pieces of functionnality
