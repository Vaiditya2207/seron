  ...
  for (const auto &line : std::views::split(str, std::string_view{"\n"})) {
    ...
    if (marker != inlineCommentMarkers.end()) {
      ...
      if (!scope) {
        ...
        return std::unexpected("Mixing @seron and @raycast keys is not allowed");
      }
      ...
      if (kv->k == "keywords") {
        if (kv->scope != "@seron") {
          return std::unexpected("keywords field is only supported in @seron scope");
        }
        std::string buf{kv->v};
      ...
      if (kv->k == "exec") {
        if (kv->scope != "@seron") {
          return std::unexpected("exec field is only supported in @seron scope");
        }
        std::string buf{kv->v};
      ...
      if (kv->k == "terminal") {
        if (kv->scope != "@seron") {
          return std::unexpected("terminal field is only supported in @seron scope");
        }
