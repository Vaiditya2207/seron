# Seron browser integration

This subdirectory contains:

- native bridge between browser extensions and seron daemon
...
The browser extension uses [Native messaging](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/Native_messaging) to communicate with the seron daemon.
...
The recommended way of installing the manifests is to use the provided CMake install rule to install seron. This rule will automatically install both manifests in the correct system locations (root access required).

### Manual native host manifest install

Copy the right manifest template from `./native-host/` and change the placeholder values.

For firefox, you only need to replace `@NATIVE_HOST_BIN` with the absolute path to the `seron-browser-link` executable (e.g: `/usr/local/libexec/seron/seron-browser-link`). Then copy the manifest at `/usr/lib/mozilla/native-messaging-hosts/com.seron.seron.json`.

For chromium you will also need to change the allowed origins, more on that below. The manifest needs to be copied at `/etc/chromium/native-messaging-hosts/com.seron.seron.json`.
...
Then modify the native host manifest (likely at `/etc/chromium/native-messaging-hosts/com.seron.seron.json`) and change the origin in the `allowed_origins` array to use the local extension ID that was automatically generated upon loading the unpacked extension.

Once that is done, reloading the extension should automatically establish a connection with seron if the seron server is running.
...
For firefox, you only need to replace `@NATIVE_HOST_BIN` with the absolute path to the `seron-browser-link` executable (e.g: `/usr/local/libexec/seron/seron-browser-link` or `/usr/libexec/seron/seron-browser-link`). Then copy the manifest at `/usr/lib/mozilla/native-messaging-hosts/com.seron.seron.json`.
...
git clone https://github.com/seronhq/seron
cd ./seron/src/browser-extension
sudo make firefox

cat > /lib/mozilla/native-messaging-hosts/com.seron.seron.json <<EOF
{
  "name": "com.seron.seron",
  "description": "Seron Native Messaging Host",
  "path": "/usr/libexec/seron/seron-browser-link",
  "type": "stdio",
  "allowed_extensions": [
    "$AMO_EMAIL_ADDRESS"
  ]
...
cd ./build/firefox
sed 's|"id": "seron@seron\.com"|"id": ""$AMO_EMAIL_ADDRESS"",\n      "data_collection_permissions": \{\n      "required": \["none"\],\n      "optional": \[\]\n      }|' -i ./manifest.json
web-ext sign
```
...
If that's any helpful, here is a very simple schema of how communication between seron and the browser works:
