...
static std::filesystem::path socketPath() {
  return std::filesystem::path(getenv("XDG_RUNTIME_DIR")) / "seron" / "seron.sock";
}
...
static int entrypoint() {
  if (isatty(STDIN_FILENO)) {
    std::println(
        std::cerr,
        "This executable is meant to be started by the seron browser extension using native host messaging "
  ...
  // Forward focusTab/closeTab events from seron to the browser extension
  ...
  // Listen for incoming events from seron server
  Frame seronFrame;

  seronFrame.setHandler([&](std::string_view message) {
    std::println(std::cerr, "got message from seron: {}", message);
    if (auto res = client.route(message); !res) {
      std::println(std::cerr, "Failed to route seron message: {}", res.error());
    }
  });
  ...
  while (true) {
    ...
    if (fds[1].revents & (POLLHUP | POLLERR)) {
      std::println(std::cerr, "seron connection closed (hangup/error), exiting...");
      break;
    }
    ...
    if (fds[1].revents & POLLIN) {
      if (!seronFrame.readPart(fds[1].fd)) {
        std::println(std::cerr, "The connection to the seron daemon broke, exiting...");
        break;
      }
    }
  }
