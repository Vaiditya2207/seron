...
It is spawned by the `seron server` command.
...
The `version` field of the `package.json` file is not meaningful. The manager is bundled with every seron release and thus shares the same version as the release it's built in.
