...
bool CliServerCommand::run(CLI::App *) {
  ...
  if (pingRes) {
    ...
    std::println(std::cerr, "Killing existing seron server (pid {})...", pingRes->pid);
  ...
  const auto path = seron::findHelperProgram("seron-server");

  if (!path) {
    std::println(std::cerr, "Could not find seron-server binary.");
    return false;
  }
  ...
  char *argv[] = {strdup("seron-server"), strdup(opts.c_str()), nullptr};

  if (execv(path->c_str(), argv) != 0) {
    std::println(std::cerr, "Failed to exec seron-server: {}", strerror(errno));
    return false;
  }
