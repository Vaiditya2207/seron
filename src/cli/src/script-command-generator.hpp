  ...
  static std::string generate(std::string_view title, Language lang, script_command::OutputMode mode) {
    ...
    static const std::unordered_map<Language, LanguageInfo> info = {
      ...
      {Language::Python, {
        ...
        "# @seron.title {title}\n"
        "# @seron.mode {mode}\n"
        "# @seron.exec [\"/usr/bin/env\", \"python3\"]\n"
        "\n"
        "print(\"Hello world!\")",
        "Python"
      }},
      ...
      {Language::TypeScript, {
        ...
        "// @seron.title {title}\n"
        "// @seron.mode {mode}\n"
        "// @seron.exec [\"/usr/bin/env\", \"ts-node\"]\n"
        "\n"
        "console.log('Hello world!');",
        "TypeScript"
      }},
