...
public:
  std::string make_help(const CLI::App *app, std::string name, CLI::AppFormatMode mode) const override {
    ...
    if (mode == CLI::AppFormatMode::Normal && app->get_parent() == nullptr) {
      out << "\n";
      out << "Learn more about Seron:        " << BLUE << "https://seron.com/docs" << RESET << "\n";
      out << "Join our Discord community:      " << CYAN << "https://seron.com/discord" << RESET << "\n";
    }
...
public:
  ...
  std::string description() const override { return "Launch or focus an app from seron"; }
...
class CliPing : public AbstractCommandLineCommand {
  ...
  std::string description() const override { return "Ping the seron server"; }
...
class ToggleCommand : public AbstractCommandLineCommand {
  ...
  std::string description() const override { return "Toggle the seron window"; }
  ...
  bool run(CLI::App *) override {
    ...
    if (auto res = cli::IpcClient::sendDeeplink("seron://toggle", query); !res) {
...
class OpenCommand : public AbstractCommandLineCommand {
  ...
  std::string description() const override { return "Open the seron window"; }
  void setup(CLI::App *app) override { app->add_option("-q,--query", m_query, "Set search query"); }
  ...
  bool run(CLI::App *) override {
    ...
    if (auto res = cli::IpcClient::sendDeeplink("seron://open", query); !res) {
...
class CloseCommand : public AbstractCommandLineCommand {
  ...
  std::string description() const override { return "Close the seron window"; }

  bool run(CLI::App *) override {
    if (auto res = cli::IpcClient::sendDeeplink("seron://close"); !res) {
...
class DMenuCommand : public AbstractCommandLineCommand {
  ...
  bool run(CLI::App *) override {
    m_req.rawContent = seron::slurp(std::cin);
...
class VersionCommand : public AbstractCommandLineCommand {
  ...
  bool run(CLI::App *) override {
    std::cout << "Version " << SERON_GIT_TAG << " (commit " << SERON_GIT_COMMIT_HASH << ")\n"
              << "Build: " << BUILD_INFO << "\n"
              << "Provenance: " << SERON_PROVENANCE << "\n";
    return true;
  }
};
...
public:
  ...
  void setup(CLI::App *app) override {
    app->alias("link");
    app->add_option("link", link, "The deeplink to open. See https://docs.seron.com/deeplinks")->required();
  }
...
int CommandLineApp::run(int ac, char **av) {
  ...
  // i.e seron seron://extensions/seron/clipboard/history
  if (ac == 2) {
    std::string arg = av[1];
    ...
    const bool hasScheme =
        std::ranges::any_of(std::initializer_list{"seron", "raycast", "com.raycast"}, pred);
...
int CommandLineInterface::execute(int ac, char **av) {
  ...
  if (Environment::isAppImage() && !ignoreAppImageWarning) {
    std::cerr
        << rang::fg::red
        << "Running Seron directly as an AppImage is not officially supported.\nThe AppImage version is "
           "intended to be installed using the script: https://docs.seron.com/install/script\nIf you "
